...
interface ClientDetail {
    ...
    preferred_barber_id: number | null; active: boolean;
}
interface Props { client: ClientDetail | null; barbers: BarberOption[]; }

export default function ClientForm({ client, barbers }: Props) {
    ...
    const { data, setData, post, patch, processing, errors } = useForm({
        ...
        notes:               client?.notes ?? '',
        preferred_barber_id: client?.preferred_barber_id ? String(client.preferred_barber_id) : '__none__',
        active:              client?.active ?? true,
    });

    function submit(e: React.FormEvent) {
        e.preventDefault();
        const payload = { ...data, preferred_barber_id: data.preferred_barber_id === '__none__' ? null : data.preferred_barber_id };
        if (isEdit) patch(`/barbershop/clients/${client.id}`, { data: payload } as any);
        else post('/barbershop/clients', { data: payload } as any);
    }
        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4">
                ...
                <form onSubmit={submit} className="flex flex-col gap-6">
                    <Card>
                        <CardHeader className="pb-3"><CardTitle className="text-base">Datos personales</CardTitle></CardHeader>
                        <CardContent className="grid gap-4 sm:grid-cols-2">
                            <div className="flex flex-col gap-1.5 sm:col-span-2">
                                <Label htmlFor="name">Nombre completo *</Label>
                                <Input id="name" placeholder="Nombre del cliente" value={data.name} onChange={e => setData('name', e.target.value)} required />
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <Select value={data.preferred_barber_id} onValueChange={v => setData('preferred_barber_id', v)}>
                            ...
                            <div className="flex flex-col gap-1.5">
                                ...
                                <Input id="preferred_style" placeholder="Ej: Fade bajo, barba perfilada" value={data.preferred_style} onChange={e => setData('preferred_style', e.target.value)} />
                            </div>
                            <div className="flex flex-col gap-1.5 sm:col-span-2">
                    ...
                    <div className="flex justify-end gap-3">
                        <Link href="/barbershop/clients"><Button type="button" variant="outline">Cancelar</Button></Link>
                        <Button type="submit" disabled={processing}>
                            {processing ? <><Spinner className="mr-1" />Guardando…</> : isEdit ? 'Actualizar Cliente' : 'Crear Cliente'}
                        </Button>
                    </div>
                </form>
            </div>
